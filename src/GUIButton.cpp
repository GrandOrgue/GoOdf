...
void GUIButton::write(wxTextFile *outFile) {
	if ((m_type.Contains(wxT("Divisional")) && !m_type.Contains(wxT("PrevBank")) && !m_type.Contains(wxT("NextBank"))) || (m_type.Contains(wxT("General")) && !m_type.IsSameAs(wxT("GeneralPrev")) && !m_type.IsSameAs(wxT("GeneralNext"))) || m_type.IsSameAs(wxT("ReversiblePiston"))) {
...
void GUIButton::read(wxFileConfig *cfg, bool isPiston) {
	...
	if (mouseRadius > -1 && mouseRadius <= std::max(m_mouseRectWidth, m_mouseRectHeight)) {
		...
		setMouseRadius(std::min(getWidth(), getHeight()) / 2);
	}
